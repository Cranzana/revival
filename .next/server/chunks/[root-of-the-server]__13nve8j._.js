module.exports = [
	...
	(e) => {
		...
		var m = e.i(14185),
			g = e.i(29364);
		let f = ["", "/pricing", "/sign-in", "/dashboard", "/learn", "/proxy"];
		function w() {
			let e = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
			return f.map((t) => ({
				url: `${e}${t}`,
				changeFrequency: t ? "weekly" : "daily",
				priority: "" === t ? 1 : 0.7,
				lastModified: new Date(),
			}));
		}
		e.s(["default", 0, w], 89282);
		var y = e.i(56636);
		async function E() {
			let e = await w(),
				t = (0, y.resolveRouteData)(e, "sitemap");
			return new g.NextResponse(t, {
				headers: {
		...
		let A = new t.AppRouteRouteModule({
			...
			{ workAsyncStorage: b, workUnitAsyncStorage: T, serverHooks: N } = A;
		async function P(e, t, a) {
			...
			let g = "/sitemap.xml/route";
			g = g.replace(/\/index$/, "") || "/";
			let f = await A.prepare(e, t, { srcPage: g, multiZoneDraftMode: !1 });
			if (!f)
				return (
					(t.statusCode = 400),
					t.end("Bad Request"),
			...
			let {
				...
				O = (0, o.normalizeAppPath)(g),
				j = !!(T.dynamicRoutes[O] || T.routes[_]),
				U = async () => (
			...
			if (j && !b) {
				let e = !!T.routes[_],
					t = T.dynamicRoutes[O];
				if (t && !1 === t.fallback && !e) {
					if (E.adapterPath) return await U();
					throw new R.NoFallbackError();
				}
			}
			let H = null;
			!j || A.isDev || b || (H = "/index" === (H = _) ? "/" : H);
			let I = !0 === A.isDev || !j,
				M = j && !I;
			S &&
				k &&
			...
			let D = e.method || "GET",
				...
				L =
					...
					(await A.getIncrementalCache(e, E, T, B));
...
//# sourceMappingURL=%5Broot-of-the-server%5D__13nve8j._.js.map
